...
function Cart() {
  ...
  useEffect(()=>{ 
    ...
    const fetchData = async () => {
        ...
        const result = await axios.post('http://localhost:5000/getusercart',korisnik)
  ...
  const deleteFromCart  = async (trenutniId) => {
    const korisnik = JSON.parse(localStorage.getItem('user'))
    const result = await axios.post('http://localhost:5000/deletefromcart', {korisnik,trenutniId})
